...
BridgeHealthSnapshot snapshot(const BridgeHealth &health) {
    BridgeHealthSnapshot snap{};
    with_lock(health, [&]() {
        uint64_t now = esp_timer_get_time();
        snap.last_uart_delta_ms = (now - health.last_uart_byte_us) / 1000ULL;
        snap.last_can_delta_ms = (now - health.last_can_publish_us) / 1000ULL;
        snap.parsed_samples = health.parsed_samples;
        snap.dropped_samples = health.dropped_samples;
        snap.can_errors = health.can_errors;
    });
    return snap;
}
